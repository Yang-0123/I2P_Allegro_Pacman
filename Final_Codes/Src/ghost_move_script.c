...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	*/
	...
	if (cnt >= 1) {
		...
		//game_log("cnt > 1 %d,premove = %d",counter_one,ghost->objData.preMove);
	}
	else { // for the dead end case
		ghost_NextMove(ghost, counter_one);
		//game_log("%d",counter_one);
	}
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	Directions wrong = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pacman->objData.Coord.x, pacman->objData.Coord.y);
	for(int i = 1 ; i <= 4 ; i++){
		if(i != wrong && ghost_movable(ghost, M, i, 1)){
			ghost->objData.nextTryMove = i;
			break;
		}
	}
	return;

...
void ghost_draw(Ghost* ghost) {
	...
	//Draw default image
	if(ghost->objData.nextTryMove == NONE && ghost->status != GO_IN){
		...
		*/ 
			...
			if (get_power_up_timer_tick() > 0.7*get_power_up_duration() )
			{
				...
				if ((ghost->objData.moveCD >> 4)& 1) {
					if((ghost->objData.moveCD >> 3)& 1) bitmap_x_offset = 48;
					else bitmap_x_offset = 32;
				}else{
					if((ghost->objData.moveCD >> 3)& 1) bitmap_x_offset = 16;
		...
		{
		case RIGHT:
			al_draw_scaled_bitmap(ghost->dead_sprite, 0+bitmap_x_offset, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0);
			break;
...
bool ghost_movable(const Ghost* ghost, const Map* M, Directions targetDirec, bool disallow_room) {
	...
	int ghost_x = ghost->objData.Coord.x;
	int ghost_y = ghost->objData.Coord.y;
	switch (targetDirec) 
	{
		case UP:
			ghost_y -= 1;
	...
	if (is_wall_block(M, ghost_x, ghost_y) || (disallow_room && is_room_block(M, ghost_x, ghost_y)))
		return false;

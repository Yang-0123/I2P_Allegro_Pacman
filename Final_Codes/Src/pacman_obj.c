...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	//game_log("location (%d, %d)",index_x,index_y);
	//game_log("dir %d",targetDirec);
	switch (targetDirec)
	{
		game_log("dir %d",targetDirec);
		case UP:
		...
		//game_log("location (%d, %d)",index_x,index_y);
	if ((is_wall_block(M, index_x, index_y) && !pacman_pass_wall)|| is_room_block(M, index_x, index_y))
		return false;
...
void pacman_draw(Pacman* pman) {
	...
	//Draw default image
	if(pman->objData.preMove == NONE && !game_over){
